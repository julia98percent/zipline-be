...
public class CoordinateUtil {
    ...
    public static double[] calculateBounds(double lat, double lon, int zoom) {
        // 줌 레벨에 따른 위도/경도 변화량 계산
        // 줌 레벨 12,14 기준으로 계산
        double baseLatDelta = 0.13173545; // 줌 레벨 12,14의 위도 변화량
        double baseLonDelta = 0.16033175; // 줌 레벨 12,14의 경도 변화량
        
        // 줌 레벨에 따른 스케일 조정
        double scale = Math.pow(2, 12 - zoom);
        double latDelta = baseLatDelta * scale;
        
        // 경도 변화량 계산 (위도에 따른 보정)
        double latFactor = Math.cos(Math.toRadians(lat));
        double lonDelta = baseLonDelta * scale * latFactor;
