...
public class ExcelServiceImpl implements ExcelService {
  ...
  private void checkCustomerDuplicateInExcel(List<CustomerExcelDTO> list) {
    ...
    for (CustomerExcelDTO dto : list) {
      ...
      if (!uniqueKeySet.add(key)) {
        ...
        throw new ExcelException(ExcelErrorCode.DUPLICATED_CUSTOMER, dto.getRowNum(),
            "name/phoneNo", key,
            "엑셀에 중복된 고객이 존재합니다.");
      }
    }
  ...
  private void checkCustomerDuplicateInDB(List<CustomerExcelDTO> list) {
    ...
    if (!exists.isEmpty()) {
      Map<String, Integer> rowNumMap = list.stream()
          .collect(Collectors.toMap(dto -> dto.getName() + "," + dto.getPhoneNo(),
              CustomerExcelDTO::getRowNum));
  ...
  private Map<String, Customer> prepareCustomersForProperty(List<AgentPropertyExcelDTO> list,
      User user) {
  ...
  private List<Customer> extractNewCustomers(List<AgentPropertyExcelDTO> list,
  ...
  private GeoCodeResultVO getGeoCode(int rowNum, String roadName) {
    ...
    if (!Strings.hasText(first.getAddress().getDongHName()) || !Strings.hasText(
        first.getAddress().getDongName())) {
      throw new ExcelException(ExcelErrorCode.INVALID_INPUT_VALUE, rowNum, "roadName", roadName,
  ...
  private List<Map<String, Object>> convertToDuplicatedCustomerDetails(
      List<Customer> duplicatedCustomers,
