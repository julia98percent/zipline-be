...
import com.zipline.entity.survey.*;
...
import com.zipline.service.survey.FileQuestionMapper;
import com.zipline.service.survey.SurveyAnswerFactory;
	...
	public SurveyResponseListDTO getSurveyResponses(PageRequestDTO pageRequestDTO, Long userUid) {
		...
		for (Long surveyResponseUid : groupedSurveyAnswers.keySet()) {
			...
			surveyResponseListDataDTOs.add(
					new SurveyResponseListDTO.SurveyResponseListDataDTO(surveyResponseUid, answers.get(0).getAnswer(), answers.get(1).getAnswer(),
							answers.get(0).getSurveyResponse().getCreatedAt()));
		}

		surveyResponseListDataDTOs.sort(
				Comparator.comparing(SurveyResponseListDTO.SurveyResponseListDataDTO::getSubmittedAt, Comparator.reverseOrder()));
